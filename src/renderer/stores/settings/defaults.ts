...
/**
 * Get initial settings state (used by the store)
 */
export function getInitialSettingsState(): Omit<
  SettingsState,
  | 'setTheme'
  | 'setLayoutMode'
  ...
  | 'addFavoriteTerminalTheme'
  | 'removeFavoriteTerminalTheme'
  | 'toggleFavoriteTerminalTheme'
  | 'setQuickTerminalEnabled'
  | 'setQuickTerminalButtonPosition'
  | 'setQuickTerminalModalPosition'
  | 'setQuickTerminalModalSize'
...
> {
  ...
  let defaultLocale: import('@shared/i18n').Locale = 'en';
  if (typeof navigator !== 'undefined') {
    const { normalizeLocale } = require('@shared/i18n');
    defaultLocale = normalizeLocale(navigator.language);
  }
